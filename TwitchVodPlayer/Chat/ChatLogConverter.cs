                                ...
                                while (jsonTextReader.Read()) {
                                    ...
                                    if (chatLine.content_offset_seconds != null) {
                                        if (beginTime == null || endTime == null) {
                                            ...
                                            progress = (int)Math.Round((float)(((double)chatLine.content_offset_seconds * 1000.0) - beginTime.Value.TotalMilliseconds) / (float)endTime.Value.TotalMilliseconds * 100.0f);

                                            BroadcastNewProgressConvertingChatLogEvent("Converting Chat Log...\n" + lineCount +
                                                "\nCurrent time: " + TimeSpan.FromMilliseconds(((double)chatLine.content_offset_seconds * 1000.0) - beginTime.Value.TotalMilliseconds) + @" / " + (endTime - beginTime),
                                                progress, progress);
